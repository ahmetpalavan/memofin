...
import { notificationDetail } from '../prisma/validators/notification-validator';
...
export const getUserNotifications = async ({ unseenOnly = false, cursor }: Params = {}) => {
  ...
  const unseenNotificationIds = unseenOnly
    ...
    : notifications.filter((notification) => !notification.seen).map((notification) => notification.id);
